...
// on GitHub?: Using std::sscanf() instead of std::stod() - sscanf is native to C, so I (meaning Prof Spano) had to do some minor legwork to change std::string tempString to a C-string. Code finally compiles and seems to work properly!
//// //// //// //// ////
...
std::ifstream myFile;
std::string line;
double EKGData[650000][2]; // most MIT BIH data seems to be 650k samples long. I'm not sure why, but if I declare this inside main, I get what appears to be a memory address error. Clearing up space on my HDD didn't affect anything, but reducing the size of the array allocation inside the function, or just putting the declaration outside the function like I did here seems to clear the error. I'm declaring this as a double because according to Prof Spano, most computers do their math in double by default. Even if you declare this as a float, which is smaller, your computer will first convert it to a double, do the math, then go back to float - which ends up taking longer.
